...
#import "BuyProductsController.h"
#import "JSON.h"
...
- (void)viewDidLoad
{
    ...
    if (app.device==0) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"Buy" style:UIBarButtonItemStylePlain target:self action:@selector(buyButtonClicked:)];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    customcheckout *cell=(customcheckout *)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if(cell==nil)
    {
        cell=[[customcheckout alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
    ...
    if (app.device==0) {
        ...
        cell.itemtotalLbl.text = [NSString stringWithFormat:@"الاجمالي: %.2f",(([[qtyArr objectAtIndex:indexPath.row]floatValue])*([[[app.cartArray objectAtIndex:indexPath.row] objectForKey:@"product_price"] floatValue]))];
    ...
    cell.itempriceLbl.text=[NSString stringWithFormat:@"سعر الوحدة : %@",[[app.cartArray objectAtIndex:indexPath.row] objectForKey:@"product_price"]];
    ...
    cell.removeBtn.titleLabel.text = @"حذف";
...
//            app.cartArray = [[NSMutableArray alloc]init];
            ...
            if ([strExpired isEqualToString:@"1"]) {
                UIAlertView *alertExpired= [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:@"عفوا ، لا يمكن استخدام هذا الكرمز لخصم" delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
                [alertExpired show];
            ...
            else if([strExpired isEqualToString:@"0"])
            {
                ...
                if ([strPrecentage isEqualToString:@"0"])
                {
                        ...
                        NSString *msg=[NSString stringWithFormat:@"مقدار الخصم %.2f%%",discountValue];
                        UIAlertView *alertDis= [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:msg delegate:self cancelButtonTitle:nil otherButtonTitles:@"موافق", nil];
                        [alertDis show];
                        alertDis.tag=2;
                        [alertDis release];
                }
                else if([strPrecentage isEqualToString:@"1"])
                {
                        ...
                        app.costInfo=totalLable.text;
                        NSString *msg=[NSString stringWithFormat:@"مقدار الخصم %.2f",discountValue];
                        UIAlertView *alertDis1= [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:msg delegate:self cancelButtonTitle:nil otherButtonTitles:@"موافق", nil];
                        [alertDis1 show];
                        alertDis1.tag=3;
                        [alertDis1 release];
                }
                ...
                UIAlertView *alertExpired= [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:@"لا يمكن تطبيق هذا الرمز للخصم" delegate:self cancelButtonTitle:nil otherButtonTitles:@"موافق", nil];
                [alertExpired show];
                [alertExpired release];
            }
        }
        else
        {
            strCost=totalLable.text;
            app.costInfo=totalLable.text;
            BuyProductsController *buyproductView=[[BuyProductsController alloc] initWithNibName:@"BuyProductsController" bundle:nil];
    ...
    {
        UIAlertView *alertMsg= [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:@"عربة التسوق فارغة" delegate:self cancelButtonTitle:nil otherButtonTitles:@"موافق", nil];
        [alertMsg show];
        [alertMsg release];
    }
