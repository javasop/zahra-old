   ...
   editbutton = (UIBarButtonItem *)sender;
    if(editbutton.title == @"تعديل"){
        if ([app.cartArray count]>0) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"متجر لمسة زهرة" message:@"الرجاء إضافة منتجات لعربة التسوق" delegate:self cancelButtonTitle:nil otherButtonTitles:@"موافق", nil];
            [alert show];
            [alert release];
        }
	}    
