...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] 
     ...
     self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"Cart" style:UIBarButtonItemStylePlain target:self action:@selector(AddToCart:)];
}
...
-(IBAction)AddProductToCart:(id)sender{
        ...
        if (![app.cartArray containsObject:dict]) {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Strore Module" message:@"The product added to cart successfully." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert show];
            [alert release];
        }
        else
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Strore Module" message:@"The product was already added to cart." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert show];
            [alert release];
        }
...
{ 
    ...
    if (app.device ==0 ){
        ...
        img = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 80, 80)];
    }
    else{
...
-(IBAction)tweet:(id)sender{
    ...
    if ([TWTweetComposeViewController canSendTweet]) {
        ...
        [t setInitialText:@"هذا المنتج من زهرة .."];
