...
#import "Article.h"
#import "HLBCArticleDetailsViewController.h"


@interface HLBCArticlesListViewController (Private)
- (void)updateItems:(id)sender;
- (void)updateArticleView:(NSNotification*)notification;
@end

@implementation HLBCArticlesListViewController
...
- (NSArray*)articlesList
{
    if (!_articlesList)
    {
        _articlesList = [[HEBCModelManager sharedHEBCModelManager].articlesList retain];
    }
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateArticleView:) name:kModelNotificationArticlesListDidChange object:[HEBCModelManager sharedHEBCModelManager]];
...
- (void)viewDidUnload
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kModelNotificationArticlesListDidChange object:[HEBCModelManager sharedHEBCModelManager]];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HLBCArticleDetailsViewController* detailsViewController = [[HLBCArticleDetailsViewController alloc] initWithNibName:[[self class] defaultTableViewControllerNibName] bundle:[NSBundle mainBundle] article:article];
...
- (void)updateItems:(id)sender
{
    [[HEBCModelManager sharedHEBCModelManager] updateFromServer_articlesList];
}
